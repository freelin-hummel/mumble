...
export type PreloadVoiceApi = Readonly<{
  connect: (
    options: UdpVoiceTransportConnectOptions,
  ) => Promise<UdpVoiceTransportStatus>;
  ...
  onMessage: (
    listener: (packet: UdpVoiceTransportPacket) => void,
  ) => () => void;
...
const subscribe = <TPayload>(
  ...
  return () => {
    ipcRenderer.removeListener(
      channel,
      wrappedListener as IpcListener<unknown>,
    );
  };
};
...
export const createPreloadApi = (
  ...
  const appApi: PreloadAppApi = Object.freeze({
    ...
    rememberServer: (serverAddress) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.rememberServer, serverAddress),
    disconnect: () => invoke(ipcRenderer, APP_INVOKE_CHANNELS.disconnect),
    selectChannel: (channelId) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.selectChannel, channelId),
    joinChannel: (channelId) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.joinChannel, channelId),
    sendChatMessage: (request) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.sendChatMessage, request),
    updateAudioSettings: (audio) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.updateAudioSettings, audio),
    updatePreferences: (preferences) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.updatePreferences, preferences),
    exportDiagnostics: (snapshot) =>
      invoke(ipcRenderer, APP_INVOKE_CHANNELS.exportDiagnostics, snapshot),
    onStateChanged: (listener) =>
      subscribe(ipcRenderer, APP_EVENT_CHANNELS.onStateChanged, listener),
  });

  const voiceApi: PreloadVoiceApi = Object.freeze({
    connect: (options) =>
      invoke(ipcRenderer, VOICE_INVOKE_CHANNELS.connect, options),
    ...
    onMessage: (listener) =>
      subscribe(ipcRenderer, VOICE_EVENT_CHANNELS.onMessage, listener),
    onStatus: (listener) =>
      subscribe(ipcRenderer, VOICE_EVENT_CHANNELS.onStatus, listener),
  });
...
export const toCspViolationPayload = (
  event: CspViolationEventLike,
): CspViolationPayload => ({
...
export const registerCspViolationLogging = (
  ...
  eventTarget.addEventListener?.("securitypolicyviolation", (event) => {
    ipcRenderer.send(
      ...
      toCspViolationPayload(event as CspViolationEventLike),
    );
  });
};
